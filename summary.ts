...
export const generateSummary = async () => {
  const config = safeLoad(
    await readFile(join(".", ".uptimerc.yml"), "utf8")
  ) as {
  ...
  const octokit = new Octokit({
    ...
    userAgent: config.userAgent || process.env.USER_AGENT || "Status Checker (+https://github.com/SoftCreatR/statuspage)",
  });
  ...
  .map(
    (page) =>
      `| ${
        ...
        page.slug
      }.yml](https://github.com/${owner}/${repo}/commits/main/history/${
        page.slug
      }.yml) | <img alt="Response time graph" src="./graphs/${
        page.slug
      }.png" height="20"> ${page.time}ms | ![Uptime ${
        page.uptime
      }%](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2F${owner}%2F${repo}%2Fmain%2Fapi%2F${
        page.slug
      }%2Fuptime.json)`
  )
  .join("\n")}
